...
class AuthService:
    ...
    @staticmethod
    def create_jwt_token(shop_id: str, business_id: int, phone: str) -> str:
        """
        ...
        try:
            async with pool.acquire() as conn:
                ...
                business = await conn.fetchrow(
                    "SELECT id, shop_id, name, owner_name, phone, password_hash, category FROM businesses WHERE phone = $1",
                    phone
                )
